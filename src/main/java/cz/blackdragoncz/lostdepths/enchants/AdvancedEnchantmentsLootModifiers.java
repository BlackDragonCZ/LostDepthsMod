...
public class AdvancedEnchantmentsLootModifiers extends LootModifier {
    ...
    public ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext context) {
        ...
        Map<Enchantment, Integer> ench = EnchantmentHelper.getEnchantments(fakeTool);
        ench.remove(LostdepthsModEnchantments.ADVANCED_FORTUNE.get());
        int newFortune = afLevel + 3;
        ench.put(Enchantments.BLOCK_FORTUNE, newFortune);
        EnchantmentHelper.setEnchantments(ench, fakeTool);

        ResourceLocation queriedId = context.getQueriedLootTableId();
        if (queriedId == null) return generatedLoot;
