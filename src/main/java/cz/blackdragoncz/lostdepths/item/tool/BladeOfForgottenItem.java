...
public class BladeOfForgottenItem extends SwordItem {
	...
	public boolean hurtEnemy(ItemStack itemstack, LivingEntity entity, LivingEntity sourceentity) {
		...
		if (sourceentity.getHealth() > sourceentity.getMaxHealth() * 0.5f) {
			float damage = entity.getMaxHealth() * 0.3f;
			entity.hurt(WeaponDamageHelper.trueDamage(entity, sourceentity, "death.attack.blade_of_forgotten"), damage);
		} else {
			float damage = entity.getMaxHealth() * 0.6f;
			entity.hurt(WeaponDamageHelper.genericDamage(entity, sourceentity, "death.attack.blade_of_forgotten"), damage);
		}
