...
$listener.Start()
Write-Host "JEI Designer Server running on http://localhost:${port}" -ForegroundColor Cyan
...
while ($listener.IsListening) {
    ...
    try {
        ...
        elseif ($method -eq "GET" -and $path -eq "/api/list-textures") {
            $textures = @()
            $folders = @("screens", "gui", "block", "item")
        ...
        # --- API: Save registry ---
        elseif ($method -eq "POST" -and $path -eq "/api/save-registry") {
            $data = Read-RequestBody $req
            $regPath = "$PSScriptRoot\registry.json"
